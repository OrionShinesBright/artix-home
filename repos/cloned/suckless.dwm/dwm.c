...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_text(drw, x, 0, bh, bh, 0, (selmon->alttag ? tagsalt[i] : tags[i]), urg & 1 << i);
		if (occ & 1 << i)
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			drw_text(drw, x, 0, w, bh, mid - 150, m->sel->name, 0);
			if (m->sel->isfloating)
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("dwm.statusbordercolor", status_border);
        XRDB_LOAD_COLOR("dwm.statusbgcolor", status_bg);
        XRDB_LOAD_COLOR("dwm.statusfgcolor", status_fg);
      }
    }
  }
